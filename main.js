...
function loadData() {
  for (i = 0; i < 100; i++) usuario.push(new Usuario());
}
...
function graph(select, join, where) {
  ...
  join.forEach((j) => {
    ...
    lastTable = j.table;
    nodeArray.forEach((t) => {
      if (op.params.includes(t.params)) op.children.push(t);
    });
    nodeArray.push(op);
  });
  ...
  for (let i = 0; i < nodeArray.length; i++) {
    tt = [];
    if (
      nodeArray[i].operation == "SELEÇÃO" ||
      nodeArray[i].operation == "PROJEÇÃO"
    ) {
      for (let j = 0; j < i; j++) {
        if (nodeArray[j].operation == "TABLE") tt.push(nodeArray[j].params);
      }
      console.log(
        `operação: ${nodeArray[i].operation} em: ${tt}, com: ${nodeArray[i].params}`
      );
    }
  }
  for (let k = 0; k < nodeArray.length; k++) {
    if (
      nodeArray[k].operation == "JUNÇÃO" ||
      nodeArray[k].operation == "PRODUTO CARTESIANO"
    )
      console.log(
        `operação: ${nodeArray[k].operation} em: ${nodeArray[k].params}, com ${nodeArray[k].params}`
      );
  }
...
function parseProjection(command) {
  ...
  let checker = (arr, target) =>
    target.every(
      (v) =>
        arr.includes(v) ||
        arr.includes(v.toLowerCase) ||
        arr.includes(v.toUpperCase)
    );
  ...
  if (
    ...
    command.forEach((e) => {
      ...
      if (s) params.push(e);
        ...
        if (keywords.includes(e)) sParams = false;
        if (sParams) subQuery.push(e);
      }
      if (e.toLowerCase() == "select") s = true;
    });
    subQuery.shift();
